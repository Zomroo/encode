...
def start_command_handler(client, message):
    client.send_message(chat_id=message.chat.id, text="Welcome to encode bot! to encode the image reply an image with /en command\nand to decode image plz reply the encoded image with /dy command")
